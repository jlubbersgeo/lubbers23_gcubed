...
soft_predictions = pd.DataFrame(vcs_load.predict_proba(iodp_data.loc[:,myfeatures]), columns = vcs_load.classes_)
soft_predictions.insert(0,'predicted_class',vcs_load.predict(iodp_data.loc[:,myfeatures]))
...
groups = {
    ...
    "749": ["U1417B-11H-2W_147-149","U1417C-11H-6W_85-87", "U1417B-11H-4W_5-7","U1417D-13H-4W_141-143"],  # 749ka
}
...
gl = ax.gridlines(crs = ccrs.PlateCarree(),draw_labels = True,linewidth = 1, linestyle = '--', color = 'gray',alpha = .5)
...
cbar = fig.colorbar(scatter, shrink = .3, ax = ax, ticks = [y.to_numpy().min(),y.to_numpy().max()],orientation = 'horizontal',anchor = (0.95,-20))
cbar.set_ticklabels(['Low', 'High'])

ax.text(.676,.15, 'Cumulative Probability', fontsize = 16, transform = ax.transAxes)
# u1417 lat: 56°57.5996′N lon: 147°6.5985′W
ax.plot(
    -147.11, 56.959, marker="*", ms=12, mec="k", mfc="k", transform=ccrs.PlateCarree()
)
ax.text(
    -147.11 - 3, 56.959 - .5, "U1417", transform=ccrs.PlateCarree(), fontsize=18
)
...
ax.text(
    -144.492 - 3, 58.777 - .5, "U1418", transform=ccrs.PlateCarree(), fontsize=18
)
ax.text(
    -180, 57, "Bering Sea", transform=ccrs.PlateCarree(), fontsize=20
)
ax.text(
    -155, 53, "Gulf of Alaska", transform=ccrs.PlateCarree(), fontsize=20
)
ax.text(
    -158, 62, "Alaska", transform=ccrs.PlateCarree(), fontsize=20
)
plt.savefig("{}\\final_IODP_probababilities_map.pdf".format(export_path),bbox_inches = 'tight')
plt.show(block = False)    
...
for g in tqdm(["27", "591", "570-2"]):
    ...
    for coords in diag_indices:
        hist, bins = np.histogram(training_data.loc[:,vars[coords[0]]],bins = 20,density = True)
    ...
    for coords in lower_indices:
        ...
        training_data.loc[best_soft_prediction, [vars[coords[1]], vars[coords[0]]]].pyroplot.density(
            ax=ax[coords[0], coords[1]],
            bins=100,
            contours=[.67,.95],
            colors=["navy", "blue"],
        ...
        if second_best_prediction:
            training_data.loc[second_best_prediction, [vars[coords[1]], vars[coords[0]]]].pyroplot.density(
            ax=ax[coords[0], coords[1]],
            bins=100,
            contours=[.67,.95],
    ...
    legend_elements = [
        ...
        Patch(facecolor=train_pred_color, edgecolor="none", label=f"{best_soft_prediction} training data (Best)"),
        Patch(facecolor=iodp_color, edgecolor="none", label=f"{g} ka\nprediction: {best_soft_prediction} ")
    ]
    if second_best_prediction:
        # custom legend
        legend_elements = [
            Patch(facecolor=train_color, edgecolor="none", label="All training data"),
            Patch(facecolor=train_pred_color, edgecolor="none", label=f"{best_soft_prediction} training data (Best)"),
            Patch(facecolor=train_second_color, edgecolor="none", label=f"{second_best_prediction} training data (2nd Best)"),
            Patch(facecolor=iodp_color, edgecolor="none", label=f"{g} ka\nprediction: {best_soft_prediction} ")
        ]
    ...
    ax[1,3].set_title(f"n = {soft_predictions.loc[groups[g],:].shape[0]}",loc = 'left', fontsize=12,)
    mpl_defaults.left_bottom_axes(ax[1,3])
    
    fig.set_facecolor('w')
    plt.savefig("{}\\{}_prediction_pairplot.pdf".format(export_path,g), bbox_inches="tight")
    if g == "570-2":
        plt.show(block = True)
    else:
